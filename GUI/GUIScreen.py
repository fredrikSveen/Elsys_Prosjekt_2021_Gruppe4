...
def Avslutt(): # Åpner varslingsvindu
    ...
    avsluttSpill = Button(w, text="End game",command=A, font=("Arial Bold", 10),bg = knapp)
    avsluttSpill.place(relx = 0.6, rely = 0.4)
    
    tilbake = Button(w, text="Back to the game", command=closeW, font=("Arial Bold", 10),bg = knapp)
    tilbake.place(relx = 0.2, rely = 0.4)

def window1(): # Åpner første vindu
    x = Label(window, text="Chose number of rounds and stones per team", font=("Arial Bold", 20), bg = bakgrunn)
    ...
    opp2 = Button(window, text="\u2B99", command=pilOpp2, font=("Arial Bold", 30), bg = knapp) # Oppknapp steiner
    opp2.place(relx = 0.6, rely = 0.35)
    
    ned2 = Button(window, text="\u2B9b", command=pilNed2, font=("Arial Bold", 30), bg = knapp) # Nedknapp steiner
    ned2.place(relx = 0.6, rely = 0.53)
    
    start = Button(window, text="Start", command=Start, font=("Arial Bold", 25),bg = knapp) # Startknapp
...
def window2(): # Vinduet under spill
    ...
    l = Label(window, text=f"Round {str(rundenr)}", font=("Arial", 50, 'bold italic'),bg = bakgrunn)
    l.place(relx = 0.4)

    avslutt = Button(window, text="Quit", command=Avslutt, font=("Arial Bold", 30),bg = knapp)
    ...
    if winner == 'blue':
        ...
        lag1.place(relx = 0.05, rely = 0.2)
        lag2 = Label(window, text="Team Orange", fg = 'darkorange', bg = bakgrunn, font=("Arial bold", 40)) 
        lag2.place(relx = 0.45, rely = 0.2)         
    ...
    def s(): # Funksjon til knapp som øker antall steiner kastet ved trykk på knapp, samt reduserer antall steiner igjen på hvert lag
        ...
        if winner == "blue":
            ...
            if (stones % 2 == 0):
                ...
                lag1 = Label(window, text="Team Blue", bg = bakgrunn, font=("Arial bold", 40))  
                lag1.place(relx = 0.05, rely = 0.2)
                lag2 = Label(window, text="Team Orange", fg = 'darkorange', font=("Arial bold", 40),bg = bakgrunn) 
                lag2.place(relx = 0.45, rely = 0.2)
            else:
                stones2 -= 1
                team2stones["text"] = str(value2 - 1)
    ...
    stonesButton=Button(window, text="Stones", command=s, bg = knapp) # "Øke antall steiner"-knapp
    ...
    regretStone = Button(window, text="Regret stone", command=regret, bg = knapp)
...
def window3(): # Vindu med resultater
    ...
    def w3_1(): # vindu 3 versjon 1
        ...
        def a2(): # Åpner et "sikker på at du vil avslutte"-vindu
            ...
            avsluttSpill2 = Button(w2, text="End game",command=a3, font=("Arial Bold", 10), bg = knapp) # Avsluttknapp i det lille vinduet
    ...
    def w3_2(): # vindu 3 versjon 2
        fortsett = Button(window, text="Continue", command=w4, bg=knapp) # Fortsettknapp
...
def window4():
    ...
    global rundenr
    rundenr = 1
    ns = Button(window, text="New game", command=nyttSpill, font=("Arial Bold", 30), bg = knapp) # "Nytt spill"-knapp
    ns.place(relx=0.35, rely=0.8)
    if sc1 > sc2:
         vinnerText = Label(window, text="The winner is Team Blue", fg = 'blue', font=("Arial Bold", 40), bg = bakgrunn)
         vinnerText.place(relx=0.1, rely=0.3)
    elif sc1 < sc2:
         vinnerText = Label(window, text="The winner is Team Orange", fg = 'orange', font=("Arial Bold", 40), bg = bakgrunn)
         vinnerText.place(relx=0.1, rely=0.3)
    else: 
         vinnerText = Label(window, text="It's a tie", font=("Arial Bold", 50), bg = bakgrunn)
